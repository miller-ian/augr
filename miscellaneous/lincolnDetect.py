...
me = "koolaid109845646565@gmail.com"
my_password = "12345678wow"
you = "6128048403@txt.att.net" # you='6128048403@txt.att.net'
...
# the length of the box around detections
objectPixels = 1
objectDetected = ''
objectCoordinates = (0.0, 0.0)
returnList = [('DROID', (42.4593, -71.2670))]
...
def get_gps_reading():
    #return (42.360262, -71.087451)
    return (42.4593, -71.2670)
...
#        data = ser.readline()
#    except:
#        print("not getting gps data")
#        return (42.356794, -71.095843)
#    print("loading")
#        
#    if data[0:6] == '$GPGLL':
...
def determine_absolute_location(start, distance, bearing):
    adjustedDistance = distance/3280.84
    d = geopy.distance.geodesic(kilometers = adjustedDistance)
    dest = d.destination(point=start, bearing=0)
    lat = dest[0]
    lon = dest[1]
    return (lat, lon)
...
def overlay_on_image(display_image, object_info):
    ...
    if labels[class_id] == "person":
        ...
        html = '<html><body><p>There\'s a person behind you</p></body></html>'
    ...
    objectPixels = box_right - box_left
    distance = calculate_distance(focalLength, realHeight, imagePixels, objectPixels, sensorHeight)
    objectDetected = label_text
    objectCoordinates = determine_absolute_location(get_gps_reading(), distance, get_bearing())
    returnList.append((objectDetected[0:len(objectDetected)-6], objectCoordinates))
...
# all the work.
def main():
    global resize_output, resize_output_width, resize_output_height, returnList
    ...
    graph_filename = 'ssdMobileNetGraph'
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        #returnList = [(objectDetected[0:len(objectDetected)-6], objectCoordinates)]
        if index > 22:
            # html rendering:
            #mf.map_fol(returnList)
            typeString = 'DROID'
            if len(returnList) > 1:
                typeString = '^a-f-'
            returnList = [('DROID', (42.4592, -71.2670))]
            
            
            index = 0
            CoT = "<?xml version = '1.0'   standalone = 'yes'?>\n <event version = '2.0'\n uid = 'J-0133'" + str(index) + "\n type = '" + typeString + "'\n time = " + str(datetime.utcnow()) + "\n <detail> \n </detail> \n <point lat = " + str(returnList[0][1][0]) + " lon = " + str(returnList[0][1][1]) + "\n </event>\n"
            print(CoT)
